...
class ESPCN(object):
    ...
    def build_model(self):
        ...
        if self.is_train:
            ...
            '''
            data = load_data(self.is_train, self.test_img, self.is_face)
            input_ = imread(data[0])
            print data[0]
            print "here is the shape " + str(input_.shape)
            self.h, self.w, c = input_.shape
            self.images = tf.placeholder(tf.float32, [None, self.h, self.w, self.c_dim], name='images')
            self.labels = tf.placeholder(tf.float32, [None, self.h * self.scale, self.w * self.scale, self.c_dim],
                                         name='labels')
    ...
    def model(self):
        ...
        conv3 = tf.nn.conv2d(conv2, self.weights['w3'], strides=[1, 1, 1, 1], padding='SAME') + self.biases[
            'b3']  # This layer don't need ReLU
    ...
    def train(self, config):
        ...
        print (config.checkpoint_dir + " is the new checkpoint being used")
